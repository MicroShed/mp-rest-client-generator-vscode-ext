...
export async function specifyYamlChoice(): Promise<string> {
  let yamlChoice: string = vscode.workspace
    .getConfiguration("microprofile.generator")
    .get<string>("defaultYamlChoice");
  if (!yamlChoice) {
    yamlChoice = await vscode.window.showQuickPick(
      [GENERATE_FROM_YAML_FILE, GENERATE_FROM_YAML_URL],
      {
        ignoreFocusOut: true,
...
export async function specifyTargetFile(): Promise<vscode.Uri> {
  ...
  let fileUri: vscode.Uri = await openDialogForFile({
...
export async function specifyYamlPath(): Promise<string> {
  const defaultYamlPath: string = vscode.workspace
    .getConfiguration("microprofile.generator")
    .get<string>("defaultYamlPath");
...
async function openDialogForFile(
  ...
  const result: vscode.Uri[] = await vscode.window.showOpenDialog(
...
export async function getFromInputBox(
  options?: vscode.InputBoxOptions
): Promise<string> {
  return await vscode.window.showInputBox(
    Object.assign({ ignoreFocusOut: true }, options)
  );
}
